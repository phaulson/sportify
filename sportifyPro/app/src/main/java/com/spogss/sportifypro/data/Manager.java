...
import com.spogss.sportifypro.data.pojo.Event;
...
public class Manager {
    ...
    public Collection<Post> getPostsByCreator(int idCreator, int lastPostId, int numberOfPosts) throws SQLException, Exception{
        if(lastPostId == 0){
            ...
            while(result.next()){
                posts.add(new Post(result.getInt(1), result.getInt(2), result.getString(3), result.getDate(4)));
            }
            return posts;
        }
     ...
     * @throws java.sql.SQLException
     */
    public int addLocation(int idUser, Coordinate coordinates, String name, LocationType type, Date startDate, Date endDate) throws SQLException{
    ...
    public Collection<Comment> getComments(int postID, int lastCommentID, int numberOfComments) throws SQLException, Exception {
        ...
        if(lastCommentID<0){
            ...
            while(result.next()){
                comments.add(new Comment(result.getInt(1), result.getInt(2), result.getInt(3), result.getString(4), result.getTimestamp(5)));
            }
        }
